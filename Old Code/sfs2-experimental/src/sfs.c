...
int remove_entry(sfs_disk* disk, char* filepath) {
  ...
  while(current_tok != NULL) {
    ...
    do {
      ...
      if(strncmp((char*) current_tok->data, (char*) next_entry->name, strlen(current_tok->data)) == 0) {
...
/** Are entries different */
int diff_entry(sfs_disk* disk, sfs_entry* entry, uint32_t size, void* buffer) {
  if(entry->length != size) return 1;

  uint32_t sector_loc = entry->payload;
  void* bufptr = buffer;
  
  while(size > 0) {
    sfs_sector* work_sector = (sfs_sector*) (disk->root + sector_loc);
    sector_loc = work_sector->next;
    
    if(size > MAX_SECTOR_DATA_SIZE) {
      if(work_sector->crc != crc64(bufptr, MAX_SECTOR_DATA_SIZE)) return 1;
      size -= MAX_SECTOR_DATA_SIZE;
      bufptr += MAX_SECTOR_DATA_SIZE;
    } else {
      if(work_sector->crc != crc64(bufptr, size)) return 1;
      size = 0;
    }    
  }
  return 0;
...
sfs_disk* sfs_init(uint32_t size, uint8_t* mem) {
  ...
  if(size && mem) {
    ...
    uint32_t* head_ptr = (uint32_t*) (mem + sizeof(uint32_t) //align to 32bit
                                        - (sizeof(HEAD_STR) % sizeof(uint32_t))); 
    ...
    if(strncmp((char*) mem, HEAD_STR, HEAD_SIZE) != 0) { //Am I formatted?
      ...
      memcpy(root.name, (uint8_t[MAX_NAME_LEN]){'.', '\0'}, sizeof(uint8_t[MAX_NAME_LEN]));
      root.length = 0;
...
int sfs_create(sfs_disk* disk, char* filepath, uint8_t type) {
  ...
  if(type == ENTRY_FILE_TYPE) {
    ...
    memcpy(self.name, (uint8_t[MAX_NAME_LEN]){'.', '\0'}, sizeof(uint8_t[MAX_NAME_LEN]));
    self.length = 0;
...
int sfs_delete(sfs_disk* disk, char* filepath) {
  ...
  if(target->type == ENTRY_FILE_TYPE) {
    ...
    remove_entry(disk, filepath);
    return 0;
  } else if(is_empty_dir(disk, target)) {
    sfs_entry* payload = (sfs_entry*) (disk->root + target->payload);
...
int sfs_write(sfs_disk* disk, char* filepath, uint32_t size, void* buffer) {
  ...
  if(entry && diff_entry(disk, entry, size, buffer)) { //there was a file, and the files are different rename it!
    char newname[MAX_NAME_LEN];
    time_t current_time = time(NULL);
    sprintf(newname, ".%s-%ld", entry->name, current_time);
    memcpy(entry->name, newname, strlen(newname) + 1);
    entry = NULL;
  }
  ...
  uint32_t next_write = entry->payload;
  entry->length = size;
  void* bufptr = buffer;
